//! Types related to process management & Functions for completely changing TCB
...
pub struct ProcessControlBlockInner {
    ...
    /// processes(also known as threads)
    pub processes: Vec<Option<Arc<ThreadControlBlock>>>,
...
impl ProcessControlBlockInner {
    ...
    /// the count of processes(threads) in this process
    pub fn thread_count(&self) -> usize {
        self.processes.len()
    }
    /// get a process with tid in this process
    pub fn get_process(&self, tid: usize) -> Arc<ThreadControlBlock> {
        self.processes[tid].as_ref().unwrap().clone()
    }
}
...
impl ProcessControlBlock {
    ...
    pub fn new(elf_data: &[u8]) -> Self {
        ...
        let process_control_block = Self {
            ...
            inner: unsafe {
                UPSafeCell::new(ProcessControlBlockInner {
                    trap_cx_ppn,
                    base_size: user_sp,
                    process_cx: TaskContext::goto_trap_return(kernel_stack_top),
        ...
        let trap_cx = process_control_block.inner_exclusive_access().get_trap_cx();
    ...
    pub fn spawn(self: &Arc<Self>, path: &str) -> Option<Arc<Self>> {
        ...
        let ret = Arc::new(ProcessControlBlock::new(get_bin_data_by_name(name).unwrap()));
        let mut parent_inner = self.inner_exclusive_access();
...
/// process status: UnInit, Ready, Running, Exited
