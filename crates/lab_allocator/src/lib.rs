...
type DefaultByteAllocator = slab::SlabByteAllocator;
...
pub struct NewSlabAllocator(Mutex<GlobalAllocator>);

impl NewSlabAllocator {
    pub const fn new() -> Self {
        Self(Mutex::new(GlobalAllocator::new()))
    }
    pub fn lock(&self) -> MutexGuard<'_, GlobalAllocator> {
        self.0.lock()
    }
}

unsafe impl GlobalAlloc for NewSlabAllocator {
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        if let Ok(ptr) = GlobalAllocator::alloc(&self.0.lock(), layout) {
            ptr.as_ptr()
        } else {
            alloc::alloc::handle_alloc_error(layout)
        }
    ...
    unsafe fn dealloc(&self, ptr: *mut u8, layout: Layout) {
        GlobalAllocator::dealloc(&self.0.lock(), NonNull::new(ptr).expect("dealloc null ptr"), layout)
    }
}
